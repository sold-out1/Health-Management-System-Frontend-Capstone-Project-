  ...
  <div class="container">
    <div class="top-header">
      ...
      <div class="nav-right">
        ...
        <div class="top-add-btn" @click="saveHealthNews">
          <i class="el-icon-plus"></i> 新增健康资讯
        </div>
      </div>
    </div>
    ...
    <!-- 新增/修改健康资讯抽屉 -->
    <el-drawer
      :title="dialogControlOperation ? '新增健康资讯' : '修改健康资讯'"
      size="40%"
      :visible.sync="drawer"
      :direction="direction"
      ...
      <div style="padding:10px 30px 10px 0;">
        <el-form ref="healthNews" :model="healthNews" label-width="80px">
          ...
          <el-form-item label="封面">
            <div>
              <img style="width:60%;height:170px;border-radius:5px;" v-if="healthNews.cover" :src="healthNews.cover || ''" alt="">
              <el-upload class="avatar-uploader" :action="fileUploadApi" :show-file-list="false" :on-success="handleImageSuccess">
          ...
          <el-form-item label="内容">
            <Editor @on-listener="onListener" :receiveContent="healthNews.content" height="300px" :api="fileUploadApi"/>
          </el-form-item>
          <el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      drawer: false,
      direction: 'rtl',
      healthNews: { id:null,typeId:null,cover:'',title:'',content:'',summary:'',createTime:null },
      healthNewsTypes: [],
      dialogControlOperation:true,
      id:null,
      apiResult:{ data:[], total:0 },
      healthNewsQueryDto:{ current:1,size:10,role:null,username:null },
  ...
  methods: {
    async onUpdateHealthNews(){ const {message}=await this.$axios.put('/health-news/update',this.healthNews); this.$message(message); this.handleClose(); this.fetchFreshData(); },
    async onSaveHealthNews(){ const {message}=await this.$axios.post('/health-news/save',this.healthNews); this.$message(message); this.fetchFreshData(); this.handleClose(); },
    onListener(content){ this.healthNews.content=content; },
    saveHealthNews(){ this.drawer=true; },
    handleClose(){ this.drawer=false; this.dialogControlOperation=true; this.healthNews={ id:null,typeId:null,cover:'',title:'',content:'',summary:'',createTime:null }; },
    async fetchHealthNewsTypes(){ const {data}=await this.$axios.get('/health-news/fetchHealthNewsTypes'); this.healthNewsTypes=data; },
    handleImageSuccess(res){ if(res.code===200) this.healthNews.cover=res.data; this.$notify({title:'封面上传',type:res.code===200?'success':'error',message:res.code===200?'上传成功':res.data,position:'bottom-right',duration:1000}); },
    listener(text){ this.healthNewsQueryDto.title=text; this.fetchFreshData(); },
    async fetchFreshData(){ const {data,total}=await this.$axios.post('/health-news/list',this.healthNewsQueryDto); this.apiResult.data=data; this.apiResult.total=total; },
    handleSizeChange(size){ this.healthNewsQueryDto.size=size; this.healthNewsQueryDto.current=1; this.fetchFreshData(); },
    handleCurrentChange(current){ this.healthNewsQueryDto.current=current; this.fetchFreshData(); },
    async handleEdit(row){ const {data}=await this.$axios.get(`/health-news/${row.id}`); this.healthNews={...data}; this.drawer=true; this.dialogControlOperation=false; },
    handleDelete(row){ this.dialogDeletedVisible=true; this.id=row.id; },
    async confirmDeleted(){ const {code}=await this.$axios.delete(`/health-news/${this.id}`); if(code===200){ this.$notify.success({title:'健康资讯删除',message:'删除成功',position:'bottom-right',duration:1000}); this.dialogDeletedVisible=false; this.id=null; this.fetchFreshData(); }}
  }
}
</script>
