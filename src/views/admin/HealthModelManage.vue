  ...
  <div class="container">
    <div class="top-header">
      ...
      <div class="nav-right">
        ...
        <div class="top-add-btn" @click="savehealthModel">
      ...
      <el-table :data="apiResult.data" border stripe style="width:100%">
        <el-table-column prop="iconUrl" width="80" label="图标">
          <template #default="scope">
            <img style="width: 30px;height: 30px;border-radius: 4px;" :src="scope.row.iconUrl" alt="">
          </template>
        </el-table-column>

        <el-table-column prop="name" width="150" label="模型名">
          <template #default="scope">
    ...
    <!-- 新增/修改模型抽屉和删除弹窗保持不变 -->
    <el-drawer :title="isCreateMode ? '新增健康模型' : '修改健康模型'" size="40%" :visible.sync="drawer"
      ...
      <div style="padding: 10px 30px 10px 0;">
        <el-form ref="healthModel" :model="healthModel" label-width="80px">
          <el-form-item label="*图标">
            <div>
              <img v-if="iconUrl" style="width: 60px;height: 60px;border-radius: 5px;" :src="iconUrl" alt="">
...
export default {
  ...
  data() {
    return {
      ...
      iconUrl: '',
      healthModel: {
        id: null, name: '', detail: '', iconUrl: null,
        unit: '', symbol: '', normalValue: '',
        userId: null, isGlobal: null, createTime: null
      },
      isCreateMode: true,
      id: null,
      apiResult: { data: [], total: 0 },
      healthModelQueryDto: { current: 1, size: 10, role: null, username: null },
      dialogDeletedVisible: false,
    };
  },
  created() { this.fetchFreshData(); },
  methods: {
    handleChange(obj) {
    ...
    async onUpdatehealthModel() {
      try {
        ...
        this.handleClose();
        this.fetchFreshData();
      } catch (error) { this.$message.info(error.message); }
    },
    ...
    savehealthModel() { this.drawer = true; },
    handleClose() {
      this.drawer = false; this.isCreateMode = true;
      this.healthModel = { id:null,typeId:null,cover:'',title:'',content:'',summary:'',createTime:null };
      this.iconUrl = '';
    },
    handleImageSuccess(res) {
      this.$notify({ title: '图标上传', type: res.code===200?'success':'error', message: res.code===200?'上传成功':res.data, position:'buttom-right', duration:1000 });
      if(res.code===200) this.iconUrl = res.data;
    },
    listener(text) { this.healthModelQueryDto.name = text; this.fetchFreshData(); },
    async fetchFreshData() {
      try {
        const { data, total } = await this.$axios.post('/health-model/list', this.healthModelQueryDto);
        this.apiResult.data = data; this.apiResult.total = total;
      } catch (error) { console.error('查询健康模型信息异常:', error); }
    },
    handleSizeChange(size) { this.healthModelQueryDto.size = size; this.healthModelQueryDto.current = 1; this.fetchFreshData(); },
    handleCurrentChange(current) { this.healthModelQueryDto.current = current; this.fetchFreshData(); },
    handleEdit(data) { this.healthModel = { ...data }; this.iconUrl = data.iconUrl; this.drawer = true; this.isCreateMode = false; },
    handleDelete(row) {
      this.id = row.id;
      if (!row.isGlobal) { this.$message.warning('删除公共模型可能会影响系统其他功能板块，暂时不支持公共模型的删除，祝您生活愉快'); return; }
      this.dialogDeletedVisible = true;
    },
    async confirmDeleted() {
      try {
        const { code, message } = await this.$axios.delete(`/health-model/${this.id}`);
        if(code===200){
          this.$notify.success({ title:'健康模型删除', message:'删除成功', position:'buttom-right', duration:1000 });
          this.dialogDeletedVisible=false; this.id=null; this.fetchFreshData();
        }else{ this.$message.error(message || '删除失败'); }
      } catch(error) { this.$message.error('删除健康模型数据异常'); console.error("删除健康模型数据异常：", error); }
    }
  }
};
</script>
...
.model { .pri{display:inline-block;width:6px;height:6px;border-radius:50%;background-color:rgb(51,51,51);margin-right:5px;} .pub{display:inline-block;width:6px;height:6px;border-radius:50%;background-color:rgb(148,165,34);margin-right:5px;} }
.pager{margin-block:20px;}
.operate-buttons{transition: opacity 0.3s;cursor:pointer;i{padding:8px;border-radius:6px;transition: all .5s ease;&:hover{background-color: rgb(236,237,238);}}}
.container{margin:10px 20px;}
.top-header{margin-block:10px;padding-inline:10px;border-radius:5px;display:flex;justify-content:space-between;align-items:center;.nav-left,.nav-right{display:flex;justify-content:left;align-items:center;gap:10px;}}
.top-add-btn{background: linear-gradient(90deg,#5B8FF9,#91C0F8); color:#fff; padding:6px 12px; border-radius:6px; cursor:pointer; display:flex; align-items:center; gap:4px; font-weight:500; transition:all 0.3s; &:hover{ background: linear-gradient(90deg,#91C0F8,#5B8FF9);} }
</style>
