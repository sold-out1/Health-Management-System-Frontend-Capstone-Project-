  ...
  <div class="container">
    ...
    <div>
      <el-table :data="apiResult.data" border stripe style="width:100%">
        ...
        <el-table-column prop="healthModelName" width="120" label="记录项"></el-table-column>
        ...
        <el-table-column prop="value" width="120" label="记录值">
          <template #default="scope">
            <div>{{ scope.row.value }}{{ scope.row.healthModelUnit }}</div>
          </template>
        </el-table-column>
...
export default {
  data() {
    return {
      ...
      healthRecordQueryDto: { current: 1, size: 10, healthModelId: null },
  ...
  methods: {
    ...
    async fetchHealthModelOptions() {
      try {
        ...
        this.healthModelOptions = [{ value: null, label: '全部' }, ...data];
      } catch (error) {
    ...
    handleSizeChange(size) {
      this.healthRecordQueryDto.size = size; this.healthRecordQueryDto.current = 1; this.fetchFreshData();
    },
    handleCurrentChange(current) {
      this.healthRecordQueryDto.current = current; this.fetchFreshData();
    },
    ...
    async confirmDeleted() {
      try {
        ...
        if (code === 200) {
          this.$notify.success({ title: '健康记录删除', message: '删除成功', position: 'buttom-right', duration: 1000 });
...
.model {
  display: flex; justify-content: left; align-items: center; gap: 16px; font-size: 14px;
  .pri { display:inline-block;width:10px;height:10px;border-radius:50%;background-color:rgb(57,132,7);margin-right:5px; }
  .pub { display:inline-block;width:10px;height:10px;border-radius:50%;background-color:rgb(227,74,32);margin-right:5px; }
}
...
.operate-buttons {
  ...
  i { padding:8px; border-radius:6px; transition: all .5s ease; &:hover{background-color: rgb(236,237,238);} }
}
