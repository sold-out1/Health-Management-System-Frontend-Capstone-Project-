<template>
    <div class="collection-health-container">
        <!-- 搜索区域 -->
        <div class="search-section">
            <div class="search-header">
                <i class="el-icon-search search-icon"></i>
                <span class="search-title">搜索收藏资讯</span>
            </div>
            <div class="search-input-wrapper">
                <AutoInput 
                    placeholder="输入关键词搜索健康资讯..." 
                    @listener="listener"
        ...
        <div class="content-card">
            ...
            <div class="table-container">
                ...
                >
                    ...
                    <el-table-column label="资讯标题" min-width="350">
                        <template #default="scope">
                            <div class="article-info" @click="view(scope.row)">
                                <div class="article-cover" v-if="scope.row.cover">
                                    <img :src="scope.row.cover" alt="封面" @error="handleImageError" />
                                </div>
                                <div class="article-details">
                                    ...
                                    <div class="article-summary" v-if="scope.row.summary">
                                        {{ scope.row.summary.substring(0, 60) }}...
                                    </div>
                                    <div class="article-meta">
                                        <span class="meta-item">
                                            <i class="el-icon-view"></i>
                                            {{ scope.row.viewCount || 0 }} 阅读
                                        </span>
                                        <span class="meta-item">
                                            <i class="el-icon-star-on"></i>
                                            {{ scope.row.upvoteCount || 0 }} 点赞
                                        </span>
                                        <span class="meta-item" v-if="scope.row.typeName">
                ...
                <div v-if="apiResult.data.length === 0" class="empty-state">
                    <div class="empty-content">
                        ...
                        <p class="empty-text">暂无收藏的健康资讯</p>
                        <p class="empty-subtext">发现优质内容后可以收藏起来方便以后查看</p>
                        <el-button 
                            type="primary" 
                            plain 
                            icon="el-icon-search"
        ...
        >
            <div class="dialog-content">
                ...
                <div class="dialog-text">
                    ...
                    <div class="article-preview" v-if="healthNews.title">
                        <div class="preview-title">{{ healthNews.title }}</div>
                        <!-- <div class="preview-time">
                            <i class="el-icon-time"></i>
                            收藏于 {{ formatTime(healthNews.createTime) }}
                        </div> -->
                    </div>
                </div>
            </div>
...
export default {
    ...
    data() {
        return {
            ...
            cancelLoading: false,
            cancelItemId: null,
            dialogKey: 0, // 用于强制重新创建弹窗
            isDialogOpening: false // 防止重复打开
        };
    },
    created() {
        this.fetchFreshData();
    ...
    methods: {
        ...
        batchCancelCollection() {
            this.$confirm(`确定要取消收藏选中的 ${this.selectedItems.length} 条内容吗？`, '批量操作确认', {
                ...
                try {
                    ...
                    this.$message.success(`成功取消 ${this.selectedItems.length} 条收藏`);
                    this.selectedItems = [];
                    this.fetchFreshData();
        ...
        handleImageError(event) {
            event.target.src = 'https://via.placeholder.com/80x60/6bb8ff/ffffff?text=健康资讯';
        },
